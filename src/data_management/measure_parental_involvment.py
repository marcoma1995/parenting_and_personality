...
involvement_m.columns = ['m_talks_about_things',
                         'm_asks_decisions','m_expresses_opinion'
                         ,'m_solves_problems',
                         'm_asks_opinion','m_gives_reason']
...
worries2001= youth2001.loc[:,['hhnr','persnr','syear','rj1402m','rj1402v']]
worries2002= youth2002.loc[:,['hhnr','persnr','syear','sj1402m','sj1402v']]
worries2003= youth2003.loc[:,['hhnr','persnr','syear','tj1402m','tj1402v']]
worries2004= youth2004.loc[:,['hhnr','persnr','syear','uj1402m','uj1402v']]
worries2005= youth2005.loc[:,['hhnr','persnr','syear','vj1402m','vj1402v']]
...
worries2010= youth2010.loc[:,['hhnr','persnr','syear','baj1403','baj1404']]
worries2011= youth2011.loc[:,['hhnr','persnr','syear','bbj1403','bbj1404']]
worries2012= youth2012.loc[:,['hhnr','persnr','syear','bcj1403','bcj1404']]
worries2013= youth2013.loc[:,['hhnr','persnr','syear','bdj1403','bdj1404']]
worries2014= youth2014.loc[:,['hhnr','persnr','syear','bej1403','bej1404']]
worries2015= youth2015.loc[:,['hhnr','persnr','syear','bfj1403','bfj1404']]
...
measure_matrix=parental_involvement_data.drop(['cid','syear','hid','pid'], axis = 1)
measures_clean = measure_matrix.dropna()
measures_mother = measures_clean.loc[:,['par_interested_in_performance',
       'parents_evening', 'teachers_officehours', 'outside_officehours',
       'm_talks_about_things', 'm_asks_decisions', 'm_expresses_opinion',
       'm_solves_problems', 'm_asks_opinion', 'm_gives_reason', 
       'm_talks_about_worries',
       'at_least_one_activity', 'm_help_studying']]
measures_mother_std = StandardScaler().fit_transform(measures_mother)

# Standardize measures related to fathers involvment.
measures_father = measures_clean.loc[:,['par_interested_in_performance',
       'parents_evening', 'teachers_officehours', 'outside_officehours',
       'f_talks_about_things', 'f_asks_decisions', 'f_expresses_opinion',
       'f_solves_problems', 'f_asks_opinion', 'f_gives_reason',
       'f_talks_about_worries', 'at_least_one_activity', 'f_help_studying']]
